...
export default function AdminDashboard() {
  ...
  // Function to calculate total area coverage based on bus routes
  ...
  // Handle bus selection from table
  ...
  const updateChart = (data) => {
    ...
    Object.keys(datasetsRef.current).forEach(busId => {
      if (!currentBusIds.includes(parseInt(busId))) {
        delete datasetsRef.current[busId];
      }
    });
    
    chart.data.datasets = chart.data.datasets.filter(dataset => {
      const busId = Object.keys(datasetsRef.current).find(
        id => datasetsRef.current[id] === dataset
      );
  ...
  const totalDistance = Object.values(buses).reduce((sum, bus) => sum + (bus.distance || 0), 0);
                ...
                Object.keys(buses).map((busId) => {
                  ...
                  return (
                    <tr key={busId} onClick={() => handleBusClick(busId)} className="clickable-row">
                      ...
                      <td>
                        <span className={`status ${bus.status || 'unknown'}`}>
                          {bus.status || 'unknown'}
                        </span>
                      </td>
