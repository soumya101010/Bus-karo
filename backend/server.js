...
mongoose
  .connect(process.env.MONGODB_URI || "mongodb://localhost:27017/bus-karo", {
...
function updateBusSpeed(bus) {
  ...
  if (bus.status === "accelerating") {
    ...
    if (bus.currentSpeed >= bus.targetSpeed - 0.5) {
      bus.status = "moving"
    }
    return
  }
  ...
  if (bus.status === "moving") {
    if (Math.random() < 0.1) {
      bus.targetSpeed = bus.maxSpeed - Math.random() * 15
    }
...
function getBusDetails(busId) {
  const bus = busRoutes.find((b) => b.id === Number.parseInt(busId))
  if (!bus) return null
  const nextStopInfo = getNextStopInfo(bus)
  return {
    id: bus.id,
...
app.get("/api/bus/:id", (req, res) => {
  const bus = busRoutes.find((b) => b.id === Number.parseInt(req.params.id))
...
server.listen(PORT, () => {
  ...
  console.log(`🔐 Authentication API available at http://localhost:${PORT}/api/register and /api/login`)
})
