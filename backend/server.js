...
io.on("connection", (socket) => {
  ...
  setInterval(() => {
    const updates = {};
    busRoutes.forEach(bus => {
      // loop through coords to simulate movement
      bus.current = bus.coords[bus.index];
      bus.distanceCovered += bus.speed * 2; // fake distance update
      updates[bus.id] = {
        lat: bus.current[0],
        lon: bus.current[1],
        speed: bus.speed,
      ...
      bus.index = (bus.index + 1) % bus.coords.length;
    });

    socket.emit("busUpdate", updates);
  }, 2000);
});
