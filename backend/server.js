...
// Example bus routes with coords + names + colors
const busRoutes = [
  {
    id: 1,
    ...
    current:  [
      ...
      progress: 0 // % of the way to next point
...
function updateBus(bus) {
  if (bus.index >= bus.coords.length - 1) {
    bus.index = 0;
    bus.progress = 0;
    bus.current = bus.coords[0];
    bus.totalDistance = 0; // reset after completing loop
    return;
  }
  ...
  bus.speed += Math.random() * 10 - 5;
  bus.speed = Math.max(15, Math.min(60, bus.speed));

  const speedMps = (bus.speed * 1000) / 3600;
  const step = speedMps * 1; // 1 sec
  ...
  if (bus.progress >= 1) {
    ...
    bus.totalDistance += segmentDist / 1000;
  } else {
